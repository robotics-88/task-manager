...
HelloDeccoManager::HelloDeccoManager(const double flightleg_acres,
                                     const std::string mavros_map_frame)
    : mavros_map_frame_(mavros_map_frame), flightleg_area_m2_(2023.0), elevation_init_(false) {
...
std_msgs::msg::String HelloDeccoManager::packageToTymbalHD(std::string topic, json gossip,
...
std_msgs::msg::String HelloDeccoManager::acceptFlight(json msgJson,
                                                      geometry_msgs::msg::Polygon &poly,
                                                      bool &poly_valid, double &home_elevation,
                                                      const rclcpp::Time timestamp,
                                                      bool &has_elevation) {
...
bool HelloDeccoManager::elevationInitializer(const double utm_x, const double utm_y) {
    ...
    try {
        ...
        tif_name = ament_index_cpp::get_package_share_directory("task_manager") + "/dem/" +
                   burn_unit_name + ".tif";
    } catch (...) {
    ...
    if (!boost::filesystem::exists(tif_name)) {
        RCLCPP_INFO(rclcpp::get_logger("hello_decco_manager"), "No elevation file found at: %s",
                    tif_name.c_str());
        return false;
    }
    RCLCPP_INFO(rclcpp::get_logger("hello_decco_manager"), "Elevation file found at: %s",
                tif_name.c_str());
...
bool HelloDeccoManager::getElevationChunk(const double utm_x, const double utm_y, const int width,
...
std_msgs::msg::String HelloDeccoManager::updateFlightStatus(std::string flight_status,
                                                            const rclcpp::Time timestamp) {
...
geometry_msgs::msg::Polygon HelloDeccoManager::polygonFromJson(json jsonPolygon) {
    ...
    if (jsonPolygon.size() < 3) {
        RCLCPP_WARN(rclcpp::get_logger("hello_decco_manager"),
                    "Invalid JSON polygon, returning empty");
        return polygon;
    }
...
visualization_msgs::msg::Marker HelloDeccoManager::visualizePolygon(const rclcpp::Time timestamp) {
    ...
    if (map_region_.points.size() < 3) {
        RCLCPP_WARN(rclcpp::get_logger("hello_decco_manager"),
                    "Invalid polygon for visualization, returning empty marker");
        return m;
    }
...
HelloDeccoManager::polygonToMap(const geometry_msgs::msg::Polygon &polygon) {
...
bool HelloDeccoManager::polygonToGeofence(
    const geometry_msgs::msg::Polygon &polygon,
    std::shared_ptr<mavros_msgs::srv::WaypointPush::Request> &request,
    const geometry_msgs::msg::PoseStamped drone_position) {

    if (polygon.points.size() < 3) {
        RCLCPP_WARN(rclcpp::get_logger("hello_decco_manager"),
                    "Invalid polygon, not creating geofence");
        return false;
    }
    ...
    if (!decco_utilities::isInside(polygon_map, drone_location)) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("hello_decco_manager"),
                    "Drone not inside polygon, adjusting polygon geofence");
        ...
        geometry_msgs::msg::Point32 point1, point2,
            closest_point = polygon_map.points.at(closest_point_ind);
        unsigned ind1, ind2;
        ...
        // If both lines have intersection, choose to act on the closer one by setting the other
        // 'intersection' flag to false
    ...
    if (!bg::is_valid(bg_poly, reason)) {
        RCLCPP_ERROR(rclcpp::get_logger("hello_decco_manager"),
                     "Geofence polgyon invalid due to %s, not setting geofence", reason.c_str());
        return false;
    }
