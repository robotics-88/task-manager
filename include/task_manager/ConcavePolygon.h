...
struct Vec2 {
    ...
    static float dot(Vec2 const &v1, Vec2 const &v2) { return v1.x * v2.x + v1.y * v2.y; }

    static float square(Vec2 const &v) { return dot(v, v); }

    static float cross(Vec2 const &v1, Vec2 const &v2) { return v1.x * v2.y - v1.y * v2.x; }
...
class ConcavePolygon {
    ...
    bool checkIfRightHanded(VertexArray &_verts) {
        ...
        for (unsigned int i = 0; i < _verts.size(); ++i) {
            signedArea +=
                Vec2::getSignedArea(_verts[i].position, _verts[mod(i + 1, _verts.size())].position);
        }
    ...
    bool isVertexInCone(LineSegment const &ls1, LineSegment const &ls2, Vec2 const &origin,
                        Vertex const &vert) {
    ...
    IntArray findVerticesInCone(LineSegment const &ls1, LineSegment const &ls2, Vec2 const &origin,
    ...
    bool checkVisibility(Vec2 const &originalPosition, Vertex const &vert,
    ...
    int getBestVertexToConnect(IntArray const &indices, VertexArray const &polygonVertices,
                               Vec2 const &origin) {
        if (indices.size() == 1) {
    ...
    int findFirstReflexVertex(VertexArray const &_vertices) {
        for (unsigned int i = 0; i < _vertices.size(); ++i) {
            float handedness =
                Vertex::getHandedness(_vertices[mod(i - 1, _vertices.size())], _vertices[i],
    ...
    void flipPolygon() { flipPolygon(vertices); }

    typedef std::map<int, Vertex> VertexIntMap;
    typedef std::pair<int, Vertex> VertexIntPair;

    VertexIntMap cullByDistance(VertexIntMap const &input, Vec2 const &origin,
        ...
        for (unsigned int i = 1; i < sliceVertices.size(); ++i)
            for (unsigned int j = i;
                 j > 0 && sliceVertices[j].distanceToSlice < sliceVertices[j - 1].distanceToSlice;
                 --j)
        ...
        for (unsigned int i = 1; i < sliceVertices.size(); ++i)
            for (unsigned int j = i; j > 0 && sliceVertices[j].index < sliceVertices[j - 1].index;
                 --j)
                std::swap(sliceVertices[j], sliceVertices[j - 1]);
  ...
  public:
    ...
    bool checkIfRightHanded() { return checkIfRightHanded(vertices); }
    ...
    int getNumberSubPolys() const { return subPolygons.size(); }
    ...
    int getPointCount() const { return vertices.size(); }
};
