...
class FlightControllerInterface : public rclcpp::Node {
  public:
    ...
    void setMapSlamTf(geometry_msgs::msg::TransformStamped tf) {
        map_tf_init_ = true;
        map_to_slam_tf_ = tf;
    }
    ...
    bool getDroneInitalized() {
        std::lock_guard<std::mutex> lock(init_mutex_);
        return drone_initialized_;
    }
  ...
  private:
    ...
    std::map<std::string, int> param_map_ = {
        {"EK3_SRC1_POSXY", 3}, {"EK3_SRC1_VELXY", 3}, {"EK3_SRC1_POSZ", 1},
        {"EK3_SRC1_VELZ", 3},  {"EK3_SRC1_YAW", 1},   {"VISO_TYPE", 0},
    };
