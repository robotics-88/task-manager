...
template <typename T> double distance(const T point_a, const T point_b) {
...
template <typename T> double distance_xy(const T point_a, const T point_b) {
    double dist_x = point_b.x - point_a.x;
    double dist_y = point_b.y - point_a.y;
...
// Determines if line segment between point_a and point_b has an orthogonal line segment that
// intersects the "origin" point
template <typename T>
bool intersectsOrthogonal(const T point_a, const T point_b, const T origin, T &intersection) {
    ...
    // For 0 <= t <= 1. So, check if there is a valid t which satisfies the conditions of desired
    // line segment which intersects the origin and is orthogonal to the first line segment
...
template <typename T> bool isInside(const geometry_msgs::msg::Polygon polygon, const T point) {
