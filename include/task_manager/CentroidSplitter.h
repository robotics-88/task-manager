...
class CentroidSplitter {
  ...
  public:
    CentroidSplitter(geometry_msgs::msg::Polygon const &_polygon, double _threshold)
        : original_polygon{_polygon}, area_threshold(_threshold) {
        assignArea();
        assignOppositeLine();
    }
    ...
    void assignArea() {
        ...
        for (unsigned int i = 0; i < original_polygon.vertices.size(); ++i) {
            signedArea += Vec2::getSignedArea(
                original_polygon.vertices[i].position,
    ...
    void assignOppositeLine() {
        ...
        // Create a line segment from the best fit line with large padding, so can compute line
        // segment intersections Compute opposite line
        double opposite_slope =
            -1 * original_polygon.blfs_params.at(1) / original_polygon.blfs_params.at(0);
        double opposite_intercept =
            original_polygon.centroid.y - opposite_slope * original_polygon.centroid.x;
    ...
    void halvePolygon(geometry_msgs::msg::Polygon &poly1, geometry_msgs::msg::Polygon &poly2) {
        ...
        for (unsigned nn = 0; nn < original_polygon.edges.size(); nn++) {
            std::pair<bool, Vec2> intersectionResult = LineSegment::intersects(
                original_polygon.edges.at(nn), original_polygon.opposite_line);
    ...
    void addVerticesBetweenIndices(std::vector<Vec2> &vertices, const unsigned start_ind,
                                   const unsigned end_ind) {
        unsigned len;
    ...
    void geometryFromVertices(const std::vector<Vec2> vertices,
    ...
    std::vector<geometry_msgs::msg::Polygon> getSubpolygons() { return outputPolygons; }
};
